...
public class MailServiceImpl{
     ...
     * @param files
     */
    public void sendMailWithAttachments(String mailFrom, String mailFromNick, String mailTo, String cc,String subject, String content, List<File> files) {
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        try {
            /*
     ...
     * @param imagePaths
     * @param imageId
     * 这个方法实现了正文带图片的功能。不同于前两种的是，String content是html格式的文本，里面用cid标注静态资源（本文是src='cid:image01'），
     * String[] imagePaths存储的是图片的路径，String[] imageId存储了每张图片的编号，这个编号是可以自己随便定义的，但是必须跟content里面使用的cid名称一致。
     */
