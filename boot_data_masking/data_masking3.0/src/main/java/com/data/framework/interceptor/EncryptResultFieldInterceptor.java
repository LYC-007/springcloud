...
import com.data.framework.annotation.IEncryptResultFieldStrategy;
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (annotation != null && returnValue != null) {
            ...
            if (strategyClassList.length != 0 && fieldKeyList.length == strategyClassList.length) {
                ...
                // 对结果进行处理
                try {
                    if (returnValue instanceof ArrayList<?>) {//arrayList
                        List<?> arrayList = (ArrayList<?>) returnValue;
                        for (int index = 0; index < arrayList.size(); index++) {
                            Object returnItem = arrayList.get(index);
                            if (returnItem instanceof String) {
                                List<String> stringList = (ArrayList<String>) arrayList;
