...
var chat = {
    ...
    createMsgBody: function (jsonData) {
        ...
        $("#user_list").append(li);
        //创建右边的消息主题
        let body = `<ul  class="content windowBody_active" id="body_` + jsonData.sendId + `">` +
            `</ul>`;
        $("#chatbox").append(body);
    },
    sendMessage: function () {
        ...
        if (websocket.readyState == WebSocket.OPEN) {
            var data = {};
            data.acceptId = acceptId; //接受者id
            data.message = text;//发送内容;
            data.sendId = $("#userId").val();//发送者id
            data.headImg = $("#myHeadImg").val();//发送者头像
